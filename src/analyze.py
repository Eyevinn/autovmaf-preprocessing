...
def video_analyzer(file_path, threshold=4):
  """ 
  ...
  while success:
    ...
    motion = motion_detector(curr_img_rgb=image, prev_img_rgb=prev_img, threshold=threshold)
    percentage = (motion['white'] / (motion['black'] + motion['white'])) * 100
